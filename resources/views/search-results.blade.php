...
<div style="background-image: url('{{ asset('asset/image/bg-3.jpg') }}'); background-size: cover; background-position: center;"
            ...
            <nav class="flex items-center justify-end gap-4">
                @auth
                    <a href="{{ auth()->user()->role === 'admin' ? route('admin.dashboard') : route('pengawas.dashboard') }}"
                        class="inline-block px-6 py-2 rounded-md text-sm font-medium bg-gradient-to-r from-green-600 to-green-700 text-white hover:from-green-700 hover:to-green-800 focus:outline-none focus:ring-2 focus:ring-green-500 dark:from-green-500 dark:to-green-600 dark:hover:from-green-600 dark:hover:to-green-700 transition-all duration-300 shadow-md">
                        Dashboard
                    </a>
                @else
                    <a href="{{ route('login') }}"
    ...
    <div class="flex flex-col items-center justify-center w-full px-4">
        <div class="w-full max-w-4xl text-center">
            <div class="bg-white/70 dark:bg-black/70 p-6 lg:p-8 rounded-xl text-center shadow-lg backdrop-blur-md">
                <div class="max-w-6xl mx-auto p-6">

                    @if ($results->count())
                        @foreach ($results->groupBy('id_unit') as $id_unit => $progres)
                            @php
                                $info = $progres->last();
                            @endphp

                            <div class="mb-10 border rounded-xl shadow bg-white dark:bg-gray-800 p-6">
                                <h3 class="text-xl font-semibold mb-4 text-green-700 dark:text-green-400">ID Unit:
                                    {{ $id_unit }}</h3>

                                <div class="mb-4 text-sm text-gray-700 dark:text-gray-300">
                                    <p><strong>Deskripsi:</strong> {{ $info->deskripsi ?? 'Belum tersedia' }}</p>
                                    <p class="mt-1"><strong>Tanggal Terbaru:</strong>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                                    @foreach ($progres as $item)
                                        <div
                                            class="rounded-lg overflow-hidden shadow hover:shadow-lg transition bg-white dark:bg-gray-700">
                                            @if ($item->foto)
                                                <a href="{{ asset('storage/' . $item->foto) }}" target="_blank">
                                                    <img src="{{ asset('storage/' . $item->foto) }}" alt="Foto Progres"
                                                        class="w-full h-48 object-cover hover:scale-105 transition duration-300">
                                                </a>
                                            @else
                                                <div
                                                    class="flex items-center justify-center h-48 bg-gray-200 text-gray-500 text-sm">
                                                    Tidak ada foto
                                                </div>
                                            @endif
                                        </div>
                                ...
                                {{-- Tombol Evaluasi Guest --}}
                                <div class="text-center">
                                    <a href="{{ route('guest.evaluasi.form', ['unit' => $id_unit, 'id_progress' => $info->id_progres]) }}"
                                        class="inline-block px-4 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 rounded shadow transition">
                                        Isi Evaluasi
                                    </a>
                    ...
                    @else
                        <div
                            class="p-6 bg-white dark:bg-gray-800 rounded shadow text-center text-gray-600 dark:text-gray-300">
                            Tidak ditemukan progres untuk nomor unit {{ $query }}.
                        </div>
                    @endif
